...
{
	// 실제 위젯 렌더링은 공용 FPropertyTable 로 위임. 위젯-영속 상태에 의존하는 지점만 컨텍스트로
	// 주입한다: (1) SoftObjectRef 에셋 피커(모달/Pending 버퍼 포함), (2) 편집 중 SceneComponent 의
	// 회전 캐시 적용, (3) SkeletalMesh FBX 임포트 다이얼로그.
	FPropertyTable::FContext Ctx;
	Ctx.FbxImportDialog = &SkeletalFbxImportDialog;
	Ctx.RenderSoftObject = [this](FPropertyValue& InProp) { return RenderSoftObjectPropertyWidget(InProp); };
	if (SelectedComponent && SelectedComponent->IsA<USceneComponent>())
	{
		USceneComponent* SceneComp = static_cast<USceneComponent*>(SelectedComponent);
